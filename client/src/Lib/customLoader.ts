...
export class CustomLoader extends DefaultLoader {
  ...
  public override onInitialize(engine: Engine): void {
    console.log("CustomLoader onInitialize");
  ...
  async _showPlayButton(): Promise<void> {
    (this._playbutton as HTMLButtonElement).style.display = "block";
    await Util.delay(200, this.engine?.clock);
    this._gameRootDiv.appendChild(this._playbutton as HTMLButtonElement);
    ...
    let playButtonClicked: Promise<void> = new Promise<void>(resolve => {
      const startButtonHandler = (e: Event) => {
        ...
        e.stopPropagation();
        e.preventDefault();
        (this._playbutton as HTMLButtonElement).removeEventListener("click", startButtonHandler);
        resolve();
      };
      (this._playbutton as HTMLButtonElement).addEventListener("click", startButtonHandler);
    });

    await playButtonClicked;
    this.dispose();
    (this._playbutton as HTMLButtonElement).style.display = "none";
  }
